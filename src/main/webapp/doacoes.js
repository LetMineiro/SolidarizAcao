...
function enviarFormulario(evt) {
    ...
    if (listaErros.length > 0) {
        evt.preventDefault(); // Bloqueia o envio
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // BUSCA CEP AUTOMATICAMENTE SE VIER DA URL
    const params = new URLSearchParams(window.location.search);
    const cepURL = params.get("cep");
    if (cepURL && /^\d{8}$/.test(cepURL)) {
        cepInput.value = cepURL.replace(/(\d{5})(\d{3})/, "$1-$2");
        buscarEndereco(cepURL);
    }

    // BUSCA ENDEREÇO AO SAIR DO CAMPO DE CEP
